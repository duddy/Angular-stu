...
@Component({
  selector: 'poc-category-management',
...
export class CategoryManagementComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute, private database: DataStoreService, 
  ...
  getPagedList() {
    this.database.findList$ByPage('category', this.pageNo, this.pageSize, this.totalItemCnt).snapshotChanges()
    .pipe(tap((list: any)=>{list.sort((p1,p2)=>p2.no-p1.no)}))
